...
##The app runs, but yer is being treated as a numeric and doesn't actually seem to work/change anything in the app
```{r}
ui <- dashboardPage(
...
##This appear to be the same app as above just with an extra option. Ideally this app would let you look at this data for each campus. The issue with the year slider is still here.
```{r}
ui <- dashboardPage(
...
shinyApp(ui, server)
```
##I don't see either of these apps as independent apps in your folder, unfortunately.
