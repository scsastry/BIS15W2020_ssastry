...
## Using a data matrix
Data matrices hold data of the same class
...
```{r}
regional <- colSums(harry_potter_matrix)
```


2. Add this information to the data matrix. Hint: you are adding a row, not a column.  
```{r}
all_harry_potter_matrix <- rbind(harry_potter_matrix, regional)
all_harry_potter_matrix
```
...
```{r}
springsdata <- c(spring_1, spring_2, spring_3, spring_4, spring_5, spring_6, spring_7, spring_8)
springs_matrix <- matrix(springsdata, nrow = 8, byrow = T)
springs_matrix
```
