...
output: 
  html_document: 
    keep_md: yes
---
##Please do not delete the homework text
...
##Possibly some other concerning variables? Maybe some characters that should be factors? (Country, group, etc)
...
##Notice you are overwriting your catch variable here
```{r}
fisheries_tidy2 %>%
...
##Which species are highest and lowest?
```{r}
...
##Grouping by sciname would make the output a little easire to read
```{r}
...
```
##Oh maps! Very ambitious, cool that you gave it a go!
```{r}
