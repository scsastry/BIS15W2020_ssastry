...
output: 
  html_document: 
    keep_md: yes
---
##Again, don't delete the homework text
...
##You shouldn't need this full file path, remember to set your working directory to where your files are
```{r}
...
```{r}
str(homerange)
#your code is totally fine, but "glimpse" also does this
```
...
#No problem with summing, just note that the question only wanted to know if there were any or not and so this would do that-
#is.na(homerange)
```

5. Change the class of the variables `taxon` and `order` to factors and display their levels.
#code is fine, just breaking it up to make it a little cleaner
```{r}
...
##Not a big deal, but Kinglet is the common name for the genus, the species, Regulus regulus is the Goldcrest

12. What is the `hra.mass.ratio` for an ostrich? Show your work, please. 
##Remember, you already have a dataframe with hra.mass.ratio in it, all you need to do is filter that dataframe for Ostrich (Struthio camelus)
