...
```{r}
ggplot(data = homerange, mapping = aes(x = log10.mass, y = log10.preymass)) +
  geom_jitter()+
  geom_smooth(method=lm, se=F)

# output: 	
# Removed 502 rows containing non-finite values (stat_smooth).
# Removed 502 rows containing missing values (geom_point).
# Means it removed the Nas because it didnt know how to plot them
```

```{r}
ggplot(data = homerange, mapping = aes(x = taxon, y = log10.preymass)) +
  geom_point()
#X values are categorical so scatterplot isn't the best visualization option
```
...
```{r}
ggplot(data = homerange, aes(x = trophic.guild)) + 
  geom_bar(stat = "count")
```
...
ggplot(data = small_mammals, aes(x= common.name, y= mean.mass.g)) +
  geom_bar(stat = "identity") +
